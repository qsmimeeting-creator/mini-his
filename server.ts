...
const OPD_TEMPLATE_CANDIDATES = [
  path.join(__dirname, "public", "templates", "OPDtemplate.pdf"),
...
const THAI_REGULAR_FONT_CANDIDATES = [
  path.join(__dirname, "public", "fonts", "Sarabun-Regular.ttf"),
...
const THAI_BOLD_FONT_CANDIDATES = [
  path.join(__dirname, "public", "fonts", "Sarabun-Bold.ttf"),
...
const buildOpdCoverPdf = async (patient: Partial<Patient>, requestedLayout?: unknown) => {
...
async function startServer() {
  ...
  app.post("/api/opd-cover/print", async (req, res) => {
    ...
    try {
      ...
      const { bytes: pdfBytes, layout } = await buildOpdCoverPdf(patient, req.body?.layout);
      ...
      res.setHeader("X-OPD-Cover-Layout-Signature", getOpdCoverLayoutSignature(layout));
